...
\begin{itemize}
    ...
    \item \textbf{Pre-trained Checkpoints}: The \texttt{checkpoint\_real\_robot/} directory contains pre-trained ACT policy checkpoints for all three tasks trained on real robot demonstrations, ready for direct deployment and evaluation.
\end{itemize}
...
\centering
\caption{AvoidObstacle (MULTI-HURDLES) evaluation. Success rate (successes/episodes) and mean episode length (steps). Visual disturbance: brightness 0.7, contrast 0.8, noise $\sigma=0.05$.}
\label{tab:custom_eval_generalization}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Policy} & \textbf{No disturbance} & \textbf{With disturbance} \\
