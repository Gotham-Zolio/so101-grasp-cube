...
def load_images_from_hdf5(hdf5_path: pathlib.Path, traj_idx: int = 0, max_images: int = 50, camera_name: str = None):
    ...
    Args:
        ...
        camera_name: Specific camera to load (None = all cameras, "front", "left_side", "right_side")
    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        if camera_name is not None:
            ...
            # Ignore any wrist_camera sensors (from old data collected before camera config change)
            available_sensors = list(sensor_group.keys())
            # Filter to only include the expected cameras, exclude wrist_camera
            expected_cameras = ['front', 'left_side', 'right_side']
            camera_names = [s for s in available_sensors if s in expected_cameras and 'wrist' not in s.lower()]
        
        print(f"Available sensors: {list(sensor_group.keys())}")
        print(f"Loading cameras: {camera_names} (ignoring wrist_camera from old data)")
        
        # No mapping needed - cameras are already named correctly
        sensor_name_mapping = {}
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Specific camera to visualize (front, left_side, right_side). Default: all cameras"
    )
    parser.add_argument(
        "--no-display",
        action="store_true",
